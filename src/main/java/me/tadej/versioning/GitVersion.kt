...
import me.tadej.versioning.cmd.CommandLineConverter
...
internal class GitVersion(
    ...
    private val executor: CommandLineExecutor = CommandLineExec(),
    private val converter: CommandLineConverter<String> = StringConverter()
) : Version {

    override fun versionName(): String {
        executor.execute(GIT_DESCRIBE, root).use {
            return converter.convert(it)
        }
    }

    override fun versionCode(): Int {
        executor.execute(GIT_COMMIT_COUNT, root).use {
            return converter.convert(it).toInt()
        }
    }
